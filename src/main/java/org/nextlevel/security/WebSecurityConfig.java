...
//		config.setAllowedOrigins(List.of("https://schoolmgmt.nextlevel.org/", "http://IP/"));
//		//config.setAllowedOrigins(List.of("*"));
//		config.setAllowedHeaders(List.of("Content-Type", "X-Requested-With", "Accept", "Origin", "Access-Control-Request-Method",
//				"Access-Control-Request-Headers"));
//		config.setAllowedMethods(List.of("GET", "POST", "PUT", "OPTIONS", "DELETE", "PATCH", "HEAD"));
...
//		httpSecurity.cors().configurationSource(request -> new CorsConfiguration().applyPermitDefaultValues());
		httpSecurity.cors().configurationSource(request -> {
			...
			corsCfg.addAllowedOrigin( CorsConfiguration.ALL );
			//corsCfg.setAllowedOrigins(List.of("*"));

			// If you really want to allow all methods
			corsCfg.addAllowedMethod( CorsConfiguration.ALL );
			//corsCfg.setAllowedMethods(List.of("GET", "POST", "PUT", "OPTIONS", "DELETE", "PATCH", "HEAD"));

			//corsCfg.setAllowCredentials(true);

			//corsCfg.addAllowedHeader("Content-Type, Accept, X-Requested-With, Origin, Access-Control-Request-Headers, " +
		...
		httpSecurity
			...
			.antMatchers(HttpMethod.DELETE, "/api/v2/orgs/**")
				.hasAuthority("NEXTLEVEL_SUPER_ADMIN")
			.antMatchers(HttpMethod.PUT, "/api/v1/orgs/{orgId}")
				//.hasAuthority("NEXTLEVEL_SUPER_ADMIN")
				.access("@orgSecurity.hasUserIdSpecificOrg(authentication, #orgId)")
			...
			.antMatchers(HttpMethod.DELETE, "/api/v2/schools/**")
				.hasAnyAuthority("NEXTLEVEL_SUPER_ADMIN", "ORG_ADMIN")
			.antMatchers(HttpMethod.PUT, "/api/v1/schools/{schoolId}")
				//.hasAnyAuthority("NEXTLEVEL_SUPER_ADMIN", "ORG_ADMIN")
				.access("@schoolSecurity.hasUserIdSpecificSchool(authentication, #schoolId)")
			.antMatchers(HttpMethod.GET, "/api/v1/schools/{schoolId}")
				...
				.hasAnyAuthority("NEXTLEVEL_SUPER_ADMIN", "NEXTLEVEL_USER", "ORG_ADMIN", "SCHOOL_ADMIN")
			.antMatchers(HttpMethod.GET, "/api/v1/schools/{schoolId}/**")
				//.access("@schoolSecurity.hasUserId(authentication, #schoolId)") //TODO:: specific school access
				.hasAnyAuthority("NEXTLEVEL_SUPER_ADMIN", "NEXTLEVEL_USER", "ORG_ADMIN", "SCHOOL_ADMIN")
